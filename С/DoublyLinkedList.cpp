...
bool DeleteSameElementOfList(doubly_linked_list*, char*);
...
int main() {
	...
	do {
		...
		case 10:
			...
			string = GetString();
			if (!DeleteSameElementOfList(List, string)) {
...
bool DeleteSameElementOfList(doubly_linked_list*List, char*current_string) {
	string*current = List->first;
	while (current != NULL) {
		if (!strcmp(current->String, current_string)) {
